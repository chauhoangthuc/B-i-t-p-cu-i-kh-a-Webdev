# TripManager - Hệ thống Quản lý Lịch trình & Chi tiêu Nhóm

Hệ thống quản lý lịch trình và chi tiêu nhóm tối ưu dành cho các chuyến đi. Ứng dụng tích hợp các tính năng phân quyền thành viên, đề xuất thời tiết thông minh, đồng bộ hóa thời gian thực và quyết toán tài chính nhóm tự động.
...
- [Cơ cấu tính năng so với architecture.md](#cơ-cấu-tính-năng-so-với-architecturemd)
...
* **Lưới lịch tháng thông minh (Calendar Mapping):** Ánh xạ trực tiếp thời gian diễn ra của toàn bộ các chuyến đi lên ô lịch tháng. Tự động đổi màu trực quan theo mốc thời gian thực tế: **Vàng** (Đã hoàn thành/Quá khứ), **Xanh lá** (Đang diễn ra), và **Đỏ** (Chuyến đi sắp tới).
...
* **Thuật toán quyết toán tối giản:** Sử dụng thuật toán rút gọn số giao dịch (`simplifyDebts`) giúp giảm tối thiểu số lượt chuyển khoản qua lại giữa các thành viên.
...
## Cơ cấu tính năng so với architecture.md
...
| | Quản lý hóa đơn trên Object Storage | **Chưa triển khai** | Hiện tại ảnh hóa đơn/ảnh đại diện đang được mã hóa **Base64** và lưu trữ trực tiếp vào PostgreSQL thay vì lưu lên MinIO do hạn chế phần cứng khi chạy dev. |
| | Chạy offline PWA hoàn toàn | **Hỗ trợ một phần** | Sử dụng IndexedDB để xếp hàng đợi Sync (Sync Queue) khi mất mạng, tuy nhiên tính năng ngoại tuyến đầy đủ (Offline caching toàn bộ CSDL) chưa được tối ưu hóa tối đa. |
